...
class SongModel {
  ...
  factory SongModel.fromMap(Map<String, dynamic> map) {
    // 이미지 URL에서 {song_id} 템플릿을 실제 ID로 대체
    String thumbnailUrl = map['thumbnail_url'] ?? '';
    if (thumbnailUrl.contains('{song_id}')) {
      thumbnailUrl = thumbnailUrl.replaceAll('{song_id}', map['id'] ?? '');
    }
    
    String songUrl = map['song_url'] ?? '';
    if (songUrl.contains('{song_id}')) {
      songUrl = songUrl.replaceAll('{song_id}', map['id'] ?? '');
    }
