...
1. **패키지 구조 문제**
  * models 디렉토리가 Python 패키지로 인식되지 않는 문제가 있었습니다.
  * **init**.py 파일이 없어서 발생한 문제였으며, 파일을 추가하여 해결했습니다.

2. **인증 미들웨어 관련 오류**
  * auth_middleware 임포트 경로의 오타로 인한 ImportError가 발생했습니다.
  * 정확한 임포트 경로를 지정하여 해결했습니다.
  * 이는 코드 구조가 복잡해질수록 발생하기 쉬운 문제로, 더 나은 임포트 구조가 필요할 수 있습니다.

3. **환경 변수 관리 문제**
  * Cloudinary API 키와 같은 민감한 정보가 코드에 직접 노출되는 문제가 있었습니다.
  * .env 파일을 도입하여 민감한 정보를 안전하게 관리하도록 개선했습니다.
  * 이는 보안상 중요한 이슈였으며, 환경 변수를 통한 설정 관리의 중요성을 보여줍니다.

4. **데이터베이스 관계 처리 문제**
  * 음악 삭제 시 외래 키 제약조건으로 인한 문제가 발생했습니다.
  * Favorite 테이블의 레코드를 먼저 삭제한 후 Song 레코드를 삭제하는 순서로 해결했습니다.
  * 이는 데이터베이스 설계 시 관계 설정의 중요성을 보여주는 사례입니다.

5. **API 엔드포인트 에러 처리**
  * /auth/signup 엔드포인트에서 400 에러가 발생하는 문제가 있었습니다.
  * 적절한 에러 메시지와 예외 처리를 추가하여 사용자 경험을 개선했습니다.
